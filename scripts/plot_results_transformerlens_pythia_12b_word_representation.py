...
SAVE_DIRECTORY = '/mnt/qb/work/eickhoff/esx208/arithmetic-lm/data/patching_results_pythia_12b_word_representations'
...
def take_operands_and_operators_results(intervention_list, patched_results, tokenizer):
    ...
    for idx, item in enumerate(intervention_list[:len(patched_results)]):
        ...
        for position, token in enumerate(curr_tokens):
            if token.strip() in all_operators or token.strip() in all_equal_signs or token.strip().isnumeric() or token.strip() in WORD_NUMBERS:
                print(token)
...
def main():
    ...
    conf.representation = REPRESENTATION
    ...
    # torch.save(all_resid_pre_clean_logit_diffs, os.path.join(SAVE_DIRECTORY, 'clean_diff.pkl'))
    # torch.save(all_resid_pre_corrupted_logit_diffs, os.path.join(SAVE_DIRECTORY, 'corrupted_logit_diff.pkl'))
    all_resid_pre_act_patch_results = torch.load(os.path.join(SAVE_DIRECTORY, 'resid_pre_logit_diff.pkl'))
    all_resid_pre_clean_logit_diffs = torch.load(os.path.join(SAVE_DIRECTORY, 'clean_diff.pkl'))
    all_resid_pre_corrupted_logit_diffs = torch.load(os.path.join(SAVE_DIRECTORY, 'corrupted_logit_diff.pkl'))
    ...
    # print(patched_results_operands_operators[0].shape)
    # print(all_resid_pre_clean_logit_diffs.shape)
    # print(all_resid_pre_corrupted_logit_diffs.shape)
    
    for i, item in enumerate(patched_results_operands_operators):
        print(' ' + corr_intervention_list[i].few_shots + corr_intervention_list[i].base_string, item.shape)
    
    patching_effect = compute_pe(patched_results_operands_operators[6:], all_resid_pre_clean_logit_diffs[6:], all_resid_pre_corrupted_logit_diffs[6:])
    torch.save(patching_effect.T, os.path.join(SAVE_DIRECTORY, 'patching_effect_residual_stream.pkl'))
    plt.figure()
    sns.heatmap(
        patching_effect.T,
        cmap='RdBu',
